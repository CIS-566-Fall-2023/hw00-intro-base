...
vec3 positionOffset(float time) {
    return vec3(cos(time), sin(time), 0) * 0.0;
}
...
void main()
{
    ...
    vec4 modelposition = u_Model * (vs_Pos + vec4(positionOffset(u_Time), 0.0));   // Temporarily store the transformed vertex positions for use below
