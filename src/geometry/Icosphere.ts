/* eslint-disable no-multi-assign, @typescript-eslint/no-loss-of-precision */
...
class Icosphere extends Drawable {
  ...
  create() {
    ...
    const maxIndexCount = 20 * 4 ** this.subdivisions;
    const maxVertexCount = 10 * 4 ** this.subdivisions + 2;
    ...
    let nextTriangles: Array<Uint32Array> = [];
    for (let i = 0; i < 20; ++i) {
    ...
    for (let s = 0; s < this.subdivisions; ++s) {
      ...
      const mid = (v0: number, v1: number): number => {
      ...
      for (let t = 0; t < triangles.length; ++t) {
        ...
        const v3 = mid(v0, v1); // Get or create a vertex between these two vertices
        const v4 = mid(v1, v2);
        const v5 = mid(v2, v0);
